...
def debug_sampling_snr_plot(model, device, args, epoch):
    ...
    针对配置: 1x256x256 Image, 208-dim Signal Condition.
    """
    model.eval()
    
    # === 1. 参数设置 (根据你的模型配置修改) ===
    num_samples = 4        # 采样样本数
    steps = 50             # 采样步数 (画图用50步足够，节省时间)
    dt = 1.0 / steps
    
    C, H, W = 1, 256, 256  # [修改点] 单通道，256x256 分辨率
    cond_dim = 208         # [修改点] Signal 维度
    
    # === 2. 初始化噪声 x (对应 Flow Matching 中的 t=0) ===
    # 形状: [B, 1, 256, 256]
    x = torch.randn(num_samples, C, H, W, device=device)
    
    # === 3. 构造条件 Signal ===
    # 由于是 debug，我们无法方便地从 dataloader拿真实 signal，
    # 这里生成标准正态分布的随机 signal 模拟输入。
    # 形状: [B, 208]
    random_signal = torch.randn(num_samples, cond_dim, device=device)
    
    # 你的 train_loop 中使用的是 "label" 作为 key
    ...
    logger.info(f"Running Debug Sampling for SNR Plotting (Epoch {epoch}) with Image {C}x{H}x{W} and Signal dim {cond_dim}...")

    for i in range(steps):
        t_value = i / steps
        t = torch.full((num_samples,), t_value, device=device)
        
        # 模型预测向量场 v (对于 OT path, v = x_1 - x_0)
        v = model(x, t, extra=conditioning)
        
        # === 核心：计算当前时刻的估计 SNR ===
        # 公式推导:
        # x_t = (1 - t) * x_0 (Noise) + t * x_1 (Data)
        # v_t = x_1 - x_0
        #
        # 反解 (Estimate):
        # pred_data (x_1) = x_t + (1 - t) * v_t
        # pred_noise (x_0) = x_t - t * v_t
        
        # 广播 t 以匹配 x 的形状 [B, 1, 256, 256]
        t_view = t.view(-1, 1, 1, 1)
        
        x1_pred = x + (1 - t_view) * v  # 预测的纯净图像
        x0_pred = x - t_view * v        # 预测的纯净噪声
        
        # 计算功率 (Mean Squared Error per sample)
        # 在 spatial dimensions [1, 2, 3] 上求平均
        ...
        # Euler Step update: x_{t+1} = x_t + v * dt
        x = x + v * dt
    ...
    if args.output_dir:
        try:
            ...
            plt.title(f"Sampling SNR Evolution (Epoch {epoch})\nInput: 1x256x256, Cond: 208-dim")
            plt.grid(True, which='both', linestyle='--', linewidth=0.5)
            plt.legend()
            
            #save_path = os.path.join(args.output_dir, f"snr_sampling_epoch_{epoch}.png")
...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not args.eval_only:
            ...
            if distributed_mode.is_main_process() and (epoch % 5 == 0):
                try:
