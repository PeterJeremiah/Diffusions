...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, extra):
        ...
        if self.cond_dim > 0:
            ...
            if y is not None:
                # Case A: 有条件 (Conditional)
                # y shape: [Batch, cond_dim] -> emb shape: [Batch, time_embed_dim]
                cond_emb = self.label_proj(y)
                ###############多卡
                # [DDP Hack] 强行拉入 null_emb，乘以0不影响结果
                if self.training:
                    cond_emb = cond_emb + 0 * self.null_emb.sum()                
            else:
                # Case B: 无条件 (Unconditional / Dropped)
                # 使用 null_emb 并扩展到当前 Batch 大小
                cond_emb = self.null_emb.expand(x.shape[0], -1)
                ###############多卡
                # [DDP Hack] 强行拉入 label_proj，构造假输入乘以0
                if self.training:
