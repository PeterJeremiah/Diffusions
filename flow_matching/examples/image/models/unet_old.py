...
class UNetModel(nn.Module):
    ...
    cond_dim: Optional[int] = None #增加condition维数
    ...
    def __post_init__(self):
        ...
        if self.cond_dim is not None:
            self.cond_emb = nn.Sequential(
                linear(self.cond_dim, self.time_embed_dim),
                nn.SiLU(),
                linear(self.time_embed_dim, self.time_embed_dim),
            )
...
#
        ...
        if "label" in extra:
            ...
            if self.num_classes is not None:
                assert (
                    y.shape == x.shape[:1]
                ), f"(分类模式) 标签形状 {y.shape} 与批次大小 {x.shape[:1]} 不匹配"
                emb = emb + self.label_emb(y)
            
            # 检查我们是否处于“向量条件”模式
            elif self.cond_dim is not None:
                # y 的形状是 [B, 208]
                # self.cond_emb 会将其投影到 [B, time_embed_dim]
                emb = emb + self.cond_emb(y)
        
        # (可选的 DDP  hack)
        elif self.num_classes and self.cond_dim is None: # 只在分类模式下执行 hack
             extra["label"] = torch.full(
                (x.size(0),), self.num_classes, dtype=torch.long, device=x.device
            )
