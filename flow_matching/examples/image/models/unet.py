...
class UNetModel(nn.Module):
    ...
    def __post_init__(self):
        ...
        if self.cond_dim > 0:
            # 将条件向量投影到时间嵌入维度
            self.label_proj = nn.Linear(self.cond_dim, self.time_embed_dim)
            # 定义一个可学习的空嵌入，用于无条件生成 (CFG drops)
            self.null_emb = nn.Parameter(torch.randn(1, self.time_embed_dim))
    ...
    def forward(self, x, timesteps, extra):
        ...
        if self.dataset == "lol":
            # 尝试获取条件图像
            # 在 train_loop 中，condition 被放在了 extra["label"]
            cond_img = extra.get("label", None)
            
            if cond_img is not None:
                    x = torch.cat([x, cond_img], dim=1)
            else:
                # 既然为了 CFG 把条件丢了，但这层物理卷积还是需要 6 个通道的数据才能运算
                # 所以我们必须用“全零”来填充这缺失的 3 个通道，充当“空条件”
                zero_padding = torch.zeros_like(x) 
                x = torch.cat([x, zero_padding], dim=1) # 变成 3+3=6 通道 (正确)
        hs = []
        ...
        # 逻辑分支: Imagenet / CT 任务 (向量条件投影)
        if self.dataset == "imagenet":
            # 只有当配置了 cond_dim 时才执行
            if self.cond_dim > 0:
                y = extra.get("label", None)
                
                if y is not None:
                    # Case A: 有条件输入 (Vector)
                    cond_emb = self.label_proj(y)
                    # DDP Hack
                    if self.training:
                        cond_emb = cond_emb + 0 * self.null_emb.sum()
                else:
                    # Case B: 无条件输入 (CFG drop)
                    cond_emb = self.null_emb.expand(x.shape[0], -1)
                    # DDP Hack
                    if self.training:
                        dummy_y = torch.zeros(1, self.cond_dim, device=x.device, dtype=x.dtype)
                        cond_emb = cond_emb + 0 * self.label_proj(dummy_y).sum()
